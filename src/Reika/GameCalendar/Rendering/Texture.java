...
public class Texture {
	...
	public Texture(File f, boolean clamp) {
		this(clamp);
		file = f;
	}

	public Texture(Class root, String path, boolean clamp) {
		this(clamp);
		reference = root;
		this.path = path;
	}

	private Texture(boolean clamp) {
		clampEdge = clamp;
	}
	...
	private void load() {
		try(InputStream in = this.getInputStream()) {
			...
			glID = TextureLoader.instance.allocateAndSetupTexture(img, clampEdge);
		}
		catch (Exception e) {
