...
public class CalendarRenderer {
	...
	public void draw(int sw, int sh) {
		...
		int uniEnd = 2015;//years.get(years.size()-1);
		if (JFXWindow.getGUI().getCheckbox("xmasBreak")) {
			this.drawSplitTimeWedge(t, DateStamp.xmasStart, DateStamp.xmasFull, DateStamp.xmasEnd, 0xff0000, 0x007700, uniEnd);
		}
		if (JFXWindow.getGUI().getCheckbox("summerBreak")) {
			this.drawSplitTimeWedge(t, DateStamp.summerStart, DateStamp.summerFull, DateStamp.summerEnd, 0xffff00, 0x00ffff, uniEnd);
		}
	...
	private void drawSplitTimeWedge(double t, DateStamp d1, DateStamp d2, DateStamp d3, int c1, int c2, int maxyear) {
		...
		int idx = years.indexOf(maxyear);
		double divR = this.getArcCenterlineRadiusAt(idx, a1)+arcThickness*(arcThicknessHalfFraction+0.125);
		double divX = divR*Math.cos(divAng);
		double divY = divR*Math.sin(divAng);
		for (double a = a0; a < a2; a += 0.5) {
			double ang = this.getGuiAngle(a);
			double r = this.getArcCenterlineRadiusAt(idx, a)+arcThickness*(arcThicknessHalfFraction+0.125);//MAX_THICKNESS;
			if (a >= 361)
				r -= arcThickness;
			double x = r*Math.cos(ang);
			double y = r*Math.sin(ang);
