...
/** <b>A double-precision implementation of a polygon, that, to paraphrase LWJGL.point, "looks remarkably similar to a java AWT one", except
 * for the perhaps-noticeable lack of any references to sun.awt.geom.Crossings to get around a 'restriction on required library' error.</b>
<br><br>
...
public class DoublePolygon {
	...
	void calculateBounds(double xpoints[], double ypoints[], int npoints) {
		...
		bounds = new DoubleRectangle(boundsMinX, boundsMinY, boundsMaxX - boundsMinX, boundsMaxY - boundsMinY);
	}
	 ...
	 * The bounding box is the smallest {@link DoubleRectangle} whose
	 * sides are parallel to the x and y axes of the
	 * coordinate space, and can completely contain the <code>Polygon</code>.
	 * @return a <code>DoubleRectangle</code> that defines the bounds of this
	 * <code>Polygon</code>.
	 * @since 1.1
	 */
	...
	public boolean contains(double x, double y) {
		if (npoints <= 2 || !this.getBoundingBox().contains(x, y)) {
			return false;
		}
		int hits = 0;
	...
	/* Removed due to "Access restriction" on Crossings...WTF?
	 ...
	 * @since 1.2
	 */
	/* Crossings restriction
	public boolean intersects(double x, double y, double w, double h) {
		if (npoints <= 0 || !this.getBoundingBox().intersects(x, y, w, h)) {
			return false;
		}
